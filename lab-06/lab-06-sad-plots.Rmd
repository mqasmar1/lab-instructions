...
staff_long %>%
  ...
  geom_line() +
  labs(
    title = "Relative Proportions of Educational Staff Plot",
    x = "Year",
    y = "Proportion (%)",
...
- Change 1: Emphasize part-time faculty by changing their color to red and changing all the other trendlines to be gray.

  Why: The change in percentage of Part-Time faculty is the most important finding from this graph, so making the other trendlines gray would help move the readers' eyes towards the Part-Time faculty trendline.

- Change 2: Adding annotations to the bottom of the graph would help readers identify patterns more easily.

  Why: Since there are 5 trendlines on the graph it would be helpful to summarize the graph for the reader and identify major findings, such as the big increase in percentage of part-time faculty.

- Change 3: Increasing the size of the lines, especially the part-time faculty line, would help emphasize the differences in percentage of educational staff

  Why: The trendlines would be easier to trace and help distinguish each staff type from each other. The change in proportion of part-time faculty is the most prominent observation and would be easier to read if the line was bolded.
...
staff_long %>%
  ggplot(aes(x = year, 
             ...
             group = faculty_type, 
             color = faculty_type)) +
  geom_line(aes(linewidth = faculty_type))  +
  guides(linewidth= "none") +
  scale_linewidth_manual(values=c(
    "Part-Time Faculty" = 3, 
    "Full-Time Tenured Faculty" = 2,
    "Graduate Student Employees" = 2,
    "Full-Time Non-Tenure-Track Faculty" = 2,
    "Full-Time Tenure-Track Faculty" = 2
  )) +
  annotate("text", x = 2005, y = 40, label = "Part-time faculty % increasing") +
  scale_color_manual(values = c("Part-Time Faculty" = "red", 
                               "Full-Time Tenured Faculty" = "gray10",
                               "Graduate Student Employees" = "gray40",
                               "Full-Time Non-Tenure-Track Faculty" = "gray60",
                               "Full-Time Tenure-Track Faculty" = "gray80"
                               )) +
  labs(
    title = "Relative Proportions of Educational Staff Plot",
    x = "Year",
...
I colored the part-time faculty trendline to be red while making all of the other lines shades of gray to emphasize the change in proportion of part-time faculty members. In the top right corner of the graph, I added an annotation to demonstrate that there was a positive net change in the percentage of part-time faculty out of educational staff. I also increased the line width so that the trendlines for all independent variables would be easier to follow. I think visibility is one of the most important aspects of good data visualizations, and the contrast between the bright red of the part-time faculty trendline and the rest of the trendlines helps tell the story of how part-time faculty proportions increased over time.
...
The readability of the plot could be greatly improved because many of the variables appear to overlap and there are many unnecessary features. For example, it is distracting that the pie charts are not flat. There are no axis labels or title for the chart on the left. 
...
- Problem 1: The pie charts are distracting because they are not flat (2D in spatial coordinates).

- Problem 2: There are no axis labels on the area chart.  

- Problem 3: There is no title for the area chart.

- Problem 4: The only country that is interpretable in the area chart is China while the scale is inappropriate for the rest of the countries in the study.

**Proposed improvements:** For each problem above, suggest a solution:

- Improvement 1: The pie charts should be turned into flat (2D in spatial coordinates) charts so that the charts are more intuitive and easy to read.

- Improvement 2: The area chart should have axis labels for both the x and y axis.

- Improvement 3: The area chart should have a title, which can be added inside the labs() function.

- Improvement 4: The area chart should be filtered to just include the top 20 countries in the dataset.
...
# Your code here
  ...
  geom_col() +
  labs(
    title = "Tons of fishery output by top 20 countries with the most fishery output",
    x = "Country",
    y = "Number of Tons",
    fill = "Type of fishery"
  ) +
    coord_flip()
```
...
I decided to create a variable to represent the type of fishery so that I could have a tidier dataset and so that I could use this new variable (fishery_type) to use as fill color for the graph. For the graph, I decided to use a bar chart to show both the relative proportions of aquaculture v. capture in each country and the approximate number of tons for each type of fishery. I also added axis labels for both the number of tons (x axis) and the country (y axis). I used coord_flip() so that the country names would fit on the visualization. My graph is also 2 spatial dimensions, which is different than the pie charts from before. I think my graph combines the findings from all three of the original graphs because of the fill and bar_chart type. I think having multiple levels of analysis on the same plots overall helps for a cleaner aesthetic and stronger visualization.
...
One improvement I proposed that I wasn't able to implement was improving the pie chart by making it two dimensional. I felt like the bar chart was the best way to summarize the findings from all 3 charts, but I'm still curious to explore pie charts and their benefits and disadvantages in presenting data.
