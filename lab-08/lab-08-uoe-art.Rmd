...
The warnings mean that some dates could not be converted into a year. For example, if the date of the art collected was unknown, there could not be a numeric year. We are okay with them because the years have an NA value if this is the case.
...
- Number of pieces with missing artist: 0
- Number of pieces with missing year: 784
- There are also 530 missing dates, which might overlap with missing year.

**Why might this information be missing?**

When I built the scrape_page function in Script 3, I trimmed the dataset to have only the observations which contained artist, year, and title. I did this because the script would not run with missing variables, and I thought this was a good way to get around that.
...
ggplot(data = uoe_art, aes(x = year)) +
  ...
  labs(
    title = "Distribution of Art in Edinburgh Collection by year",
    x = "Year",
    y = "Count"
  )
```
...
There is an observation of Art in the Edinburgh Collection that was created in 1700. This is unusual because most of the art is concentrated around the mid 20th century to the present.
...
# Remember to change eval=FALSE to eval=TRUE!
uoe_art %>%
  filter(year < 1800)  # Look for unreasonable years
```

**Visit the link for this piece and find the correct year:** 

Visited the Edinburgh art collection website and found that the dates for the following pieces are in ranges such as 1700-1850.

**Why didn't our code capture it correctly?**

There are 5 pieces with out of the ordinary years in the dataset: Queen Anne apron, Patchwork quilt, Queen Anne chair cover, Georgian waistcoat, and Chairseat. I think our code did not capture it correctly because the University of Edinburgh is unsure of the date. For Chair seat, it says the date is "1700?-1850?", which is extremely unusual. On the website, this matches up. It is hard to pinpoint the date of Chair seat because it is unknown, which causes this error.


**Correct the error:**
```{r fix-year, eval = TRUE}
# Remember to change eval=FALSE to eval=TRUE!
# There are no scraping errors with the pieces. All of the years are accurate, but some years are unknown. For example Chair seat was created between 1700 and 1850. This is information directly from the website and in the dataframe.
```
...
**Most common artist:** The most common artist is Emma Gillies, but for many of the pieces the artist is unknown. The number of observations with unknown artists is higher than the number of observations for a known artist.

**Number of pieces:** Emma Gillies has 157 pieces in the collection.

**Do you know this artist?** No, however I might google Emma Gillies.




**Why might the university have so many of their pieces?** The university might have so many of their pieces because her art was from the early 20th century, which is when the art collection received the most pieces and probably had the most resources.
...
**Number of pieces with "child" in the title:** 9 pieces have "child" in the title.
