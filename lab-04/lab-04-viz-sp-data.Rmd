...
**Your answer:** The Denny's dataset has `r nrow(dennys)` rows and `r ncol(dennys)` columns. Each row represents an observation of a denny's restaurant and its location. The variables are address, city, state, zip, longitude, and latitude. I found this out by running `?dennys` in the console.
...
**Your answer:** The La Quinta dataset has `r nrow(laquinta)` rows and `r ncol(laquinta)` columns. Each row represents one motel and its location. The variables are address, city, state, zip, longitude, and latitude. I found this out by running `?laquinta` in the console.
...
Using the link from above, (https://locations.dennys.com/), there are no international locations on the Denny's map, so I postulate that there are 0 Denny's locations outside the US based on the given data.
...
**Your ideas:**
- I could print out the observations that do not have their "state" variable as one of the 50 U.S. states or the city as Washington D.C.. 
- I could scrape the names of all the cities in the United States, go through the dennys/laquinta dataset and print out the observations that do not have the city variable as one of the US cities.
- I could also scrape the zip codes in the United States, go through the dennys/laquinta dataset and print out the observations that do not have any matching zip code.
...
**If there are any, which state/province codes appear?** N/A
...
I think this is unsurprising because Maine and Delaware have significantly lower populations than other states such as California or New York.
...
I got a list of states with corresponding n, name, and area variables.
...
If Washington D.C. is considered a "state", then it has the highest density of Denny's restaurants with a value of 29.3. Washington DC is a district though.
...
Yes, Mitch Hedberg's joke appears to hold in North Carolina. For most observations in North Carolina, the longitude and latitude of the Denny's location is close to the longitude and latitude of the La Quinta location, most of the time. In other words, the Denny's and La Quinta tend to be clustered together.
...
Yes, Hedberg's joke appears to hold in Texas. For every observation of a Denny's in Texas, there is almost always an observation of a La Quinta overlapping the point on the graph. The Denny's and La Quinta establishments tend to be clustered.

**Compare North Carolina and Texas: In which state does the joke seem more accurate?**

The joke seems more accurate in Texas. I postulate that this is the case because there are more observations of Denny's and La Quinta in Texas, so the pattern of the establishments being close together should be more prevalent.
